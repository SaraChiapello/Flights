...
public class FlightDAO {
	...
	public List<Airline> getAllAirlines(AirlineIdMap airlineIdMap) {
		...
		try {
			...
			while (res.next()) {
				...
				list.add(airlineIdMap.get(airline.getAirlineId()));
			}
			conn.close();
			return list;
	...
	public List<Route> getAllRoutes(AirlineIdMap airlineIdMap, AirportIdMap airportIdMap, RouteIdMap routeIdMap) {
		...
		try {
			...
			int count=0;
			while (res.next()) {
				Route route=new Route(count,res.getString("Airline"), res.getInt("Airline_ID"), res.getString("Source_airport"),
				...
				list.add(routeIdMap.get(routeId));
				count++;
			}
			conn.close();
			return list;
	...
	public List<Airport> getAllAirports(AirportIdMap airportIdMap) {
		...
		try {
			...
			while (res.next()) {
				...
				list.add(airportIdMap.get(airport.getAirportId()));
			}
			conn.close();
			return list;
		} catch (SQLException e) {
